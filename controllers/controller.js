  ...
  request('http://www.gizmodo.com/', function(error, response, html) {
    ...
    $('h1.headline').each(function(i, element) {
       ...
       result.title = $(this).children("a").text().trim();
        ...
        result.summary = "Unfortunately we can't get things working for you right now. We will keep trying!" //convert to string for error handling later
      

        // Error handling to ensure there are no empty scrapes
        if(result.title !== ""){

          // BUT we must also check within each scrape 
        ...
        // Log that scrape is working, just the content was missing parts
          ...
          console.log('Redundant Gizmodo Content. Not Saved to DB.')
        }
